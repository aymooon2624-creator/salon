...
async function init() {
  ...
  const existingServices = await prepare('SELECT COUNT(*) as count FROM services').get();
  if (existingServices.count === 0) {
    ...
    for (const s of services) {
      await prepare('INSERT INTO services (name, price, duration, icon) VALUES (?, ?, ?, ?)').run(s.name, s.price, s.duration, s.icon);
    }
  ...
  const settings = await prepare('SELECT id FROM settings WHERE id = 1').get();
  if (!settings) {
    await prepare('INSERT INTO settings (id) VALUES (1)').run();
    console.log('✅ تم إضافة الإعدادات الافتراضية');
  } else {
    console.log('ℹ️  الإعدادات موجودة مسبقاً');
  }

  const existingRewards = await prepare('SELECT COUNT(*) as count FROM loyalty_rewards').get();
  if (existingRewards.count === 0) {
    ...
    for (const r of rewards) {
      await prepare('INSERT INTO loyalty_rewards (name, description, points_required, reward_type, reward_value, service_id) VALUES (?, ?, ?, ?, ?, ?)').run(
  ...
  const existingCoupons = await prepare('SELECT COUNT(*) as count FROM coupons').get();
  if (existingCoupons.count === 0) {
    ...
    for (const c of coupons) {
      await prepare('INSERT INTO coupons (code, description, discount_type, discount_value, min_order, usage_limit, valid_from, valid_until) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(
  ...
  const existingNotifSettings = await prepare('SELECT id FROM notification_settings WHERE id = 1').get();
  if (!existingNotifSettings) {
    await prepare('INSERT INTO notification_settings (id) VALUES (1)').run();
