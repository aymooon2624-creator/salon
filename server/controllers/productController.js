...
async function getAllAdmin(req, res) {
  const { prepare } = await getDb();
  const products = await prepare('SELECT * FROM products').all();
  res.json(products);
}
...
async function create(req, res) {
  ...
  const result = await prepare('INSERT INTO products (name, price, description, icon) VALUES (?, ?, ?, ?) RETURNING id').run(
    name, price, description || '', icon || '🧴'
  );
  save();
  const product = await prepare('SELECT * FROM products WHERE id = ?').get(result.rows[0].id);
...
async function update(req, res) {
  ...
  await prepare('UPDATE products SET name = ?, price = ?, description = ?, icon = ?, is_active = ? WHERE id = ?').run(
    name || existing.name,
